...
def mozzartNba():
    try:
        ...
        for row in rows:
            ...
            print(time_of_game, playerName, spans[2].text, spans[4].text, row.find_element(By.CLASS_NAME, "odds").text)
            if (playerName != 'D.Booker'):
              bookmaker_list.append(Bookmaker("Mozzart", float(row.find_element(By.CLASS_NAME, "odds").text), float(spans[4].text),
                                  float(spans[2].text), time_of_game, playerName))
        driver.close()
